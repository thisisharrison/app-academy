...
def rock_superstars
  ...
  execute(<<-SQL)
    ...
    ORDER BY COUNT(DISTINCT styles.album) DESC
    LIMIT 1;
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      styles.style,
      SUM(tracks_count.price) / SUM(tracks_count.count) AS avg_price
    FROM 
      styles
    JOIN (
      SELECT
    ...
    ON styles.album = tracks_count.id
    GROUP BY style
    ORDER BY avg_price DESC, styles.style ASC
    LIMIT 5;
