...
def teachers_and_mobiles
  ...
  execute(<<-SQL)
    SELECT
      teachers.name,
      COALESCE(teachers.mobile, '07986 444 2266') AS mobile_no 
    FROM
      teachers;
  SQL
end
...
def teachers_and_depts
  ...
  execute(<<-SQL)
    SELECT
      teachers.name,
      COALESCE(depts.name, 'None') AS dept_name
    FROM
      teachers
    LEFT JOIN
      depts ON teachers.dept_id = depts.id;
...
def num_teachers_and_mobiles
  ...
  execute(<<-SQL)
    SELECT
      COUNT(teachers.name) AS count_teachers,
      COUNT(teachers.mobile) AS count_mobiles
    FROM
      teachers;
  SQL
end
...
def dept_staff_counts
  ...
  execute(<<-SQL)
    SELECT
      depts.name,
      COUNT(teachers.name) AS staff_count
    FROM
      depts
    LEFT JOIN
      teachers ON depts.id = teachers.dept_id
