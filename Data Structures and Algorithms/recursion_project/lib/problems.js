...
function flatten(data) {
  ...
  // if data is an arrays
  // unpack accumulator, call flatten on current, unpack result
  return data.reduce((acc, cur) => { return [...acc, ...flatten(cur)] }, []);
}
...
function fileFinder(directories, targetFile) {
  for (let key in directories) {
    // key is a file or key is a subfile, and we check the subfiles
...
function pathFinder(directories, targetFile) {
  for (let key in directories) {
    ...
    // the subfolder
    const subdir = directories[key];
    // result could would lead to targetfile or null because { file: null }
    const result = pathFinder(subdir, targetFile);
    if (result !== null) {
      // result is contains targetFile, key is the current folder name
      return key + result;
    }
  }
  return null;
