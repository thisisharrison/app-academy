...
def sum_array(array)
    ...
    first = array[0]
    rest = array[1..-1]
    return first + sum_array(rest) # if sum_array_2, first becomes original array[1] = we get array[0] + array[1] of original 
end
...
def reverse_string(str)
    ...
    return "" if str.empty? # string also has [] and #empty? methods
    ...
    # return reverse_string(last) + reverse_string(rest)
...
def flatten(data)
    ...
    flattened = []
    data.each {|ele| flattened += flatten(ele)} # for loop to pass 0-dimensional array (string) and hit base case [data] and add that to flattened
    flattened
